...
class nnpShortcuts(CommonShortcuts):
    ...
    def plot_dispersion(self, 
        ...
        CommonShortcuts.draw_inplane_dispersion(ax, kPoints, dispersions, states_toBePlotted, False, False, labelsize, ticksize, annotatesize, markersize=markersize, lattice_temperature=lattice_temperature)
        fig.tight_layout()
    ...
    def plot_carrier_densities(self, 
        ...
        # --- save all the figures to one PDF
        if PDFfilename:
            self.export_figs(PDFfilename, 'pdf')
    ...
    def kp_density_analysis(self, 
        ...
        # --- save all the figures to one PDF
        if PDFfilename: self.export_figs(PDFfilename, 'pdf')
    ...
    def calculate_overlap(self, output_folder, force_lightHole=False):
        ...
        if electron_state_is_multiband and hole_state_is_multiband:
            ...
            for nu in range(len(self.kp6_basis)):
                prod = np.multiply(np.conjugate(amplitude_h[nu,:]), amplitude_e[:])   # multiply arguments element-wise (NOT inner product of spinor vector!)
                overlap += simpson(prod, x)
        else:
