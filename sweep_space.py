...
class SweepSpace:
	...
	def has_sweep_variable(self, key):
	...
	def get_values_by_variable_name(self, key):
		...
		if key in self.data.keys():
			...
			raise KeyError(f"Sweep variable '{key}' does not exist!")
	...
	def extract_1D_line(self, sweep_var):
		...
		if not self.has_sweep_variable(sweep_var):
			raise KeyError(f"Sweep variable {sweep_var} does not exist!")
	...
	def extract_2D_plane(self, sweep_var1, sweep_var2):
		...
		if not self.has_sweep_variable(sweep_var1):
			raise KeyError(f"Sweep variable {sweep_var1} does not exist!")
		if not self.has_sweep_variable(sweep_var2):
			raise KeyError(f"Sweep variable {sweep_var2} does not exist!")
